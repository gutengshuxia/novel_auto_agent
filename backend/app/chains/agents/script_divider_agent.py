...
**一个镜头 = 一个完整的视觉单元**。判断标准是：观众在屏幕上看到的画面是否是连续、连贯的。

**必须合并的情况**：
- 环境描写 + 同一环境中的人物活动 → 合并为一个镜头（这是经典的"建立镜头"）
  - 例："破旧出租屋，墙皮剥落...陆沉坐在椅子上" → 这是一个镜头，不是两个
- 同一场景内的连续动作/描写 → 合并
- 同一人物在同一地点的连续行为 → 合并

## 必须拆分的 4 种情况（缺一不可才拆）

1. **地点变化**：从"A地"移动到"B地"（如从"出租屋"到"街道"）
2. **时间跳跃**：时间发生明显变化（如"三天后"、"到了夜晚"）
3. **重大情节转折**：情绪或事件发生根本性变化（如突然打斗、意外发生、有人闯入）
4. **视角/主体切换**：从一个人物的视角切换到另一个人物在另一个地点的活动

**注意**：仅仅因为文本分了段落、信息量较大、或描写了不同细节，都不是拆分的理由。
...
- ❌ 不要每句话一个镜头
- ❌ 不要机械地按固定行数拆分
- ❌ 不要把"环境描写"和"人物描写"拆成两个镜头（它们通常属于同一个镜头）
