...
分镜拆分结果过度拆分：将同一场景内的"环境描写"和"人物活动"拆成两个独立镜头（如"破旧出租屋"和"陆沉坐在椅子上"本应是一个建立镜头）。

### 修改内容

- **ScriptDividerAgent prompt 重构**（`backend/app/chains/agents/script_divider_agent.py`）
  - 新增"核心原则：宁少勿多，合并优先"
  - 明确"环境描写 + 同一环境中的人物活动 = 一个镜头"
  - 拆分条件从 5 条收紧为 4 条（删除"节奏需要/超过5-8行就拆"）
  - 新增禁止规则："不要把环境描写和人物描写拆成两个镜头"
  - 强调"仅仅因为文本分了段落或信息量大，不是拆分理由"
